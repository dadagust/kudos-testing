    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            if flush:
                if dry:
                    self.stdout.write(
                        self.style.WARNING('DRY-RUN: Category.objects.all().delete()')
                    )
                else:
            ...
            def upsert(node: Node, parent: Category | None = None):
                nonlocal created, existed
                slug = make_slug(node.name)
                defaults = {'name': node.name, 'parent': parent}
                if dry:
                    self.stdout.write(
                        f"[DRY] ensure Category(name='{node.name}', slug='{slug}', parent={parent})"
                    )
                    ...
                    if was_created:
                        ...
                        if obj.name != node.name or obj.parent_id != (
            ...
            if dry:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Готово. Создано: {created}, существовало: {existed}.')
                )
